...
- **`ARCIIN_HOST_DATA_DIR`** (Docker) or **`ARCIIN_DATA_DIR`** (native) must point at persistent SSD/HDD storage (default `/srv/arciin-storage/arciin`). Native and Docker installers use the same layout and migration logic — see [DOCKER.md](./DOCKER.md).
- Replace the default **Postgres** password in `docker-compose.yml` (or an override file) before exposing the host to the internet; the bundled `arciin/arciin` credential is for local/LAN only.
- Put Caddy behind your preferred DNS, reverse proxy, or TLS termination.
- Set **`ARCIIN_PUBLIC_URL`** to the URL browsers use (`http://<lan-ip>` or `https://your.domain`).
- **`SESSION_SECRET`** and **`ARCIIN_SETUP_TOKEN`**: use values generated by `./scripts/docker-setup.sh` or `./install.sh`; never commit real `.env` files.
- Docker uploads failing: `sudo chown -R 1000:1000 "$ARCIIN_HOST_DATA_DIR"`.
- Storage moves after install: desktop **Settings → Storage** (one-click migration; source data is kept).
