...
import { ARCIIN_DEFAULT_SYSTEM_INSTRUCTION } from "@/components/chat/chat-page"
...
const PROVIDERS: ProviderMeta[] = [
  ...
  },
  {
    ...
    description: "Claude Opus 4, Sonnet 4, and Haiku via the Anthropic API.",
    requiresKey: true,
    requiresBaseUrl: false,
  ...
  },
  {
    ...
    description: "Gemini 2.0 Flash, Pro, and Ultra through Google AI Studio.",
    requiresKey: true,
    requiresBaseUrl: false,
  ...
  },
  {
    ...
    logoInvert: true,
    description: "Grok-2 and Grok-3 from xAI, via the xAI API.",
    requiresKey: true,
    requiresBaseUrl: false,
  ...
  },
  {
    ...
    description: "Llama 3 and Llama 4 via Together AI, Fireworks, or any compatible endpoint.",
    requiresKey: true,
  ...
  },
  {
    ...
    description: "Qwen-Turbo, Qwen-Plus, and Qwen-Max from Alibaba Cloud.",
    requiresKey: true,
    requiresBaseUrl: false,
  ...
  },
  {
    ...
    description: "Text-to-speech and voice cloning for audio content and narration.",
    requiresKey: true,
    requiresBaseUrl: false,
...
function ProviderCard({
  ...
  return (
    <div
      ...
      style={connected ? { borderColor: "rgba(255,79,18,0.25)", background: "rgba(255,79,18,0.02)" } : undefined}
    >
      {/* Top */}
        ...
        {connected ? (
          <>
            ...
            <Button
              size="sm"
              variant="ghost"
              className="ml-auto h-7 px-2 text-zinc-400 hover:text-red-500"
...
function SystemInstructionCard() {
  ...
  function save() {
    localStorage.setItem(SYSTEM_INSTRUCTION_KEY, instruction.trim())
    setSaved(true)
    setTimeout(() => setSaved(false), 2000)
    toast.success("System instruction saved.")
  }

  function clear() {
    localStorage.setItem(SYSTEM_INSTRUCTION_KEY, "")
    setInstruction("")
    toast.success("System instruction cleared.")
  }
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-[13px] font-semibold text-foreground">System Instruction</h3>
          <p className="mt-0.5 text-[12px] leading-relaxed text-muted-foreground">
            Prepended to every chat request across all providers. Use it to define the AI&apos;s role, tone, or context about your instance.
          </p>
        </div>
      </div>
      <textarea
        ...
        placeholder={`You are a helpful assistant for Arciin, a self-hosted file and media management platform. Help users understand their libraries, files, and settings. Be concise and practical.`}
        rows={4}
        className="mt-3.5 w-full resize-none rounded-xl border border-border bg-background px-3.5 py-3 text-[13px] leading-relaxed text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-primary/20"
      />
      <div className="mt-2.5 flex items-center justify-between gap-3">
        <p className="text-[11px] text-muted-foreground">
          Stored in your browser — applies to all AI providers in this session.
        </p>
        <div className="flex shrink-0 gap-2">
          {instruction && (
            <Button
              ...
              variant="ghost"
              onClick={clear}
              className="h-7 px-3 text-[12px] text-muted-foreground hover:text-red-500"
            >
              Clear
            </Button>
          )}
          ...
          >
            {saved ? <><Check className="size-3" />Saved</> : "Save"}
          </Button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="w-full min-w-0 space-y-6 pb-8">
      <DashboardPageIntro
        ...
        description="Connect AI providers by adding your API keys. Keys are stored on this instance only. Arciin uses these profiles for AI Chat and future automations."
        stats={[
        ...
        {PROVIDERS.map((meta) => {
          ...
          return (
            <ProviderCard
              ...
              meta={meta}
              profile={profile}
              onConnect={() => setSheetMeta(meta)}
              onEdit={() => setSheetMeta(meta)}
