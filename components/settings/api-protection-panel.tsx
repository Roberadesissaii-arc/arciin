...
export function ApiProtectionPanel() {
  ...
  const ipMutation = useMutation({
    ...
    onSuccess: async (data, vars) => {
      ...
      const labels = { block: "blocked", allow: "allowlisted", unblock: "unblocked", disallow: "removed from allowlist" }
      toast.success(`${vars.ip} ${labels[vars.action]}`)
      queryClient.setQueryData<SecuritySettings>(queryKeys.securitySettings, (old) =>
        old
    ...
    onError: (err) => {
      toast.error(err instanceof Error ? err.message : "Could not update IP rule.")
    },
  })
            ...
            <PillSwitch
              ...
              onChange={() => {
                ...
                if (next && allowlist.length === 0) {
                  toast.error("Add at least one address to the allowlist before enforcing.")
                  return
                }
                save(
