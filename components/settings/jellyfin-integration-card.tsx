...
export function JellyfinIntegrationCard({ integration }: { integration: IntegrationSummary }) {
  ...
  const toggleMutation = useMutation({
    ...
    onSuccess: async (_data, enabled) => {
      ...
      if (enabled) {
        ...
        toast.success(
          ready
            ? "Jellyfin enabled. Jellyfin folders are set up in Videos, Images, and Music—new uploads will use them."
            : "Jellyfin enabled, but some folders could not be verified. Try “Repair folders” below.",
        )
      } else {
        toast.success("Jellyfin disabled. Uploads no longer auto-route to Jellyfin folders (existing folders stay).")
      }
    },
    onError: (e: Error) => toast.error(e.message || "Could not update Jellyfin."),
  })
  ...
  const repairMutation = useMutation({
    ...
    onError: (e: Error) => toast.error(e.message || "Could not repair Jellyfin folders."),
  })
