...
import { AudioSeparationService, AudioSeparatorBackend } from "@arciin/media-ai"
import {
  disconnectRunPod,
...
} from "@/services/dubbing/runpod-config"
import { testRunPodConnection } from "@/services/dubbing/runpod-connection-test"
import { loadDubbingSettings, saveSeparationMode } from "@/services/dubbing/separation-settings"
...
/**
 * The RunPod connection form.
 *
 * Every field is required together: a half-configured provider is worse than
 * none, because it makes Cloud selectable and then fails after the reader has
 * waited for it.
 */
const runpodConnectionSchema = z.object({
  apiKey: z.string().min(10).max(500),
  endpointId: z.string().min(3).max(200),
  volumeId: z.string().min(3).max(200),
  datacenter: z.string().min(2).max(60),
  s3AccessKeyId: z.string().min(3).max(500),
  s3SecretAccessKey: z.string().min(3).max(500),
})
...
export async function registerSettingsRoutes(fastify: FastifyInstance) {
   ...
   *
   * Owner and admin only: this stores billing-capable credentials and decides
   * where a user's audio is allowed to travel.
   */
  fastify.get(
    "/settings/audio-separation",
    { preHandler: requireRole(["OWNER", "ADMIN"]) },
    async (_request, reply) => {
      const separation = new AudioSeparationService([new AudioSeparatorBackend()])
      const [runpod, settings] = await Promise.all([
        getRunPodStatus(fastify.prisma),
        loadDubbingSettings(fastify.prisma),
      ])

      reply.send({
        data: {
          local: {
            available: await separation.isAvailable(),
            engine: "Demucs",
            implementation: "python-audio-separator",
            compute: "CPU",
          },
          // Never a secret: see runpod-config, where the only decrypting
          // function is the one the worker calls.
          runpod,
  ...
  fastify.put(
    "/settings/audio-separation/runpod",
    { preHandler: requireRole(["OWNER", "ADMIN"]) },
    async (request, reply) => {
      const parsed = runpodConnectionSchema.safeParse(request.body)
      if (!parsed.success) {
        reply.status(400).send({
          error: {
            code: "VALIDATION_ERROR",
            message: "Every RunPod field is required.",
            // Field names only — never the submitted values, which are secrets.
            details: { fields: Object.keys(parsed.error.flatten().fieldErrors) },
          },
        })
        return
      }

      const status = await saveRunPodConnection(fastify.prisma, parsed.data)
      reply.send({ data: status })
    },
  )

  fastify.post(
    "/settings/audio-separation/runpod/test",
    { preHandler: requireRole(["OWNER", "ADMIN"]) },
    async (_request, reply) => {
      const credentials = await getRunPodCredentials(fastify.prisma)
      if (!credentials) {
        reply.status(409).send({
          error: { code: "NOT_CONFIGURED", message: "Connect RunPod before testing it." },
        })
        return
      }

      const result = await testRunPodConnection(credentials)
      await recordRunPodTest(fastify.prisma, {
        ok: result.ok,
        at: new Date().toISOString(),
        gpuName: result.gpuName ?? null,
        cudaAvailable: result.cudaAvailable ?? null,
        separatorVersion: result.separatorVersion ?? null,
  ...
  fastify.delete(
    ...
    { preHandler: requireRole(["OWNER", "ADMIN"]) },
    async (_request, reply) => {
      // Credentials only. Dubs, cached stems and source media are the user's
      // work and survive disconnecting the provider that helped make them.
      const status = await disconnectRunPod(fastify.prisma)
      reply.send({ data: status })
    },
  )

  fastify.patch(
    "/settings/audio-separation/mode",
    { preHandler: requireRole(["OWNER", "ADMIN"]) },
    async (request, reply) => {
      const parsed = z
        .object({ mode: z.enum(["auto", "local", "cloud"]) })
        .safeParse(request.body)
      if (!parsed.success) {
        reply.status(400).send({
          error: { code: "VALIDATION_ERROR", message: "Pick auto, local or cloud." },
        })
        return
      }
      const settings = await saveSeparationMode(fastify.prisma, parsed.data.mode)
      reply.send({ data: { mode: settings.separationMode } })
    },
  )
