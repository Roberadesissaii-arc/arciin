...
import { Copy, Plus, RefreshCcw, Send, Trash2, Webhook } from "lucide-react"
...
function EndpointForm({
  ...
  const mutation = useMutation({
    ...
    onError: (e: Error) =>
      toast.error("Could not save webhook", { description: e.message || "Try again in a moment." }),
  })
            ...
            onClick={async () => {
              try {
                ...
                toast.success(mode === "create" ? "Webhook created." : "Webhook updated.", {
                  description:
                    mode === "create"
                      ? "This endpoint will now receive the selected event types."
                      : "Changes are live for future events.",
                })
              } catch (error) {
...
export function WebhooksManager() {
  ...
  const deleteMutation = useMutation({
    ...
    onError: (e: Error) =>
      toast.error("Could not delete webhook", { description: e.message || "Try again in a moment." }),
  })
  ...
  const testMutation = useMutation({
    ...
    onError: (e: Error) =>
      toast.error("Could not send test event", {
        description: e.message || "Check the endpoint URL and try again.",
      }),
  })
  ...
  return (
    <div className="space-y-6 pb-6">
      <DashboardPageIntro
        ...
        cornerDecoration={<IntroCornerIcon icon={Webhook} />}
                            ...
                            onClick={async (e) => {
                              ...
                              try {
                                ...
                                toast.success("Test delivery sent.", {
                                  description: "Check the deliveries log below for the response.",
                                })
                              } catch (error) {
