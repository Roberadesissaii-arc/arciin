...
import { notifyUploadStarted } from "@/lib/notifications/toast-actions"
...
import { useImportProgressPoll } from "@/hooks/use-import-progress-poll"
...
export function GlobalDropzoneProvider({
    ...
    async (files: File[]) => {
      ...
      if (uploadable.length === 0) {
        toast.error("No files to upload", {
          description:
            "Drop a folder with files inside, or use Upload folder — empty folders cannot be uploaded.",
        })
        return
      }
      ...
      try {
        ...
        if (clean.length > 0) {
          notifyUploadStarted(
            clean.length === 1 ? clean[0]!.name : undefined,
            clean.length > 1 ? clean.length : undefined,
          )
  ...
  async function handleResolve(resolved: DuplicateConflict[]) {
      ...
      if (conflict.resolution === "replace") {
        try {
          ...
          toast.error(`Could not remove existing ${conflict.file.name}`, {
            description: "The replacement upload was skipped. Try again.",
          })
          continue
        }
    ...
    if (toUpload.length > 0) {
      notifyUploadStarted(
        toUpload.length === 1 ? toUpload[0]!.name : undefined,
        toUpload.length > 1 ? toUpload.length : undefined,
      )
