...
const sizePx = {
  sm: 24,
  md: 32,
  /** Collapsed sidebar rail — between nav icons and full tile. */
  sidebar: 28,
  lg: 40,
  xl: 48,
} as const
...
export function ArciinSidebarWordmarkText({
  ...
  return (
    <span
      ...
      aria-label="Arciin"
    >
      Arciin<span style={{ color: "var(--arciin-accent, #ff4f12)" }}>.</span>
    </span>
  )
}
  ...
  return (
    ...
    >
      ...
      <span className={cn("text-white", textClassName)}>
        rciin<span style={{ color: "var(--arciin-accent, #ff4f12)" }}>.</span>
      </span>
    </span>
  )
}
