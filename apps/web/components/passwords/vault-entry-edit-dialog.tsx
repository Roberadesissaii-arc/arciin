...
function entryToDraft(entry: PasswordVaultEntry): DraftFields {
  return {
    name: entry.name,
...
function VaultEntryEditForm({
  ...
  const [draft, setDraft] = useState<DraftFields>(() => entryToDraft(entry))
  const [passwordEdited, setPasswordEdited] = useState(false)
  const [showEditedPassword, setShowEditedPassword] = useState(false)
  const [userHidRevealed, setUserHidRevealed] = useState(false)

  const hasStoredPassword = vaultEntryHasPassword(entry)
  const showRevealedPassword = Boolean(revealedPassword) && !userHidRevealed
  const showPassword = passwordEdited ? showEditedPassword : showRevealedPassword
  ...
  const handleEyeClick = () => {
    if (passwordEdited) {
      setShowEditedPassword((v) => !v)
      return
    }

    if (revealedPassword) {
      if (showRevealedPassword) {
        setUserHidRevealed(true)
        onClearRevealedPassword()
      } else {
        setUserHidRevealed(false)
      }
      return
    }
              ...
              placeholder={
                hasStoredPassword
                  ? showRevealedPassword
                    ? "Tap the eye to hide the saved password"
      ...
      <AlertDialogContent className="max-w-md">
        {entry ? (
          <VaultEntryEditForm
            ...
            onRequestPasswordReveal={onRequestPasswordReveal}
            onClearRevealedPassword={onClearRevealedPassword}
            busy={busy}
            onSave={onSave}
          />
        ) : null}
