...
export function PreviewAskAiSeam({
  ...
  onOpen: () => void
  onClose: () => void
  /** When set, seam opens license settings instead of Ask AI. */
  lockedLabel?: string
}) {
  ...
  return (
    <button
      ...
      aria-label={
        locked
          ? `Ask AI available on ${lockedLabel}`
          : open
            ? "Collapse Ask AI"
            : "Open Ask AI"
      }
    ...
    >
      {open ? (
        ...
        <>
          {locked ? (
            <Lock className="size-4 text-[#ff4f12]" />
          ) : (
