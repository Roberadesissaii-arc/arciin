...
export function AccessControlPanel() {
          ...
          : `Revoked ${data.revoked} session${data.revoked === 1 ? "" : "s"}.`,
        {
          description:
            data.revoked === 0
              ...
              : "Those devices will need to sign in again.",
        },
      )
      setRevokeOpen(false)
      queryClient.invalidateQueries({ queryKey: queryKeys.accessControlStatus })
  ...
  function save(patch: Partial<SecuritySettings>, label: string) {
    mutation.mutate(patch, {
      onSuccess: () => toast.success(label, { description: "Access control settings updated." }),
    })
  }
        ...
        <CardContent>
          ...
          >
            <PillSwitch
              ...
              disabled={busy}
              onChange={() => {
                const next = !d.publicSignupEnabled
                save(
          ...
          >
            <PillSwitch
              ...
              disabled={busy}
              onChange={() => {
                const next = !d.loginAlertsEnabled
                save(
