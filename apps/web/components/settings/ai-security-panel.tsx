...
export function AiSecurityPanel() {
      ...
      { [key]: enabled },
      {
        onSuccess: () => {
          toast.success(`${SETTING_LABELS[key]} ${enabled ? "enabled" : "disabled"}`, {
            description: "AI security settings updated for this instance.",
          })
        },
      },
    )
  ...
  if (isError || !data) {
    return (
      <SettingsPanelError
        message={error instanceof Error ? error.message : "Could not load AI security settings."}
                ...
                { passwordVaultAiAccess: value },
                {
                  onSuccess: () => {
                    ...
                    toast.success(`Password vault AI access: ${label}`, {
                      description: "The assistant never receives real passwords.",
                    })
                  },
                },
              )
          ...
          <PillSwitch
            ...
            onChange={() =>
              mutation.mutate(
                ...
                {
                  onSuccess: () => {
                    const nowOn = !s.passwordQueriesLocalAiOnly
                    toast.success(`Password chat local-only ${nowOn ? "enabled" : "disabled"}`, {
                      description: nowOn
                        ? "Password-related chat questions now stay on your local Ollama profile."
                        : "Password-related chat questions may use cloud AI providers again.",
                    })
                  },
                },
              )
