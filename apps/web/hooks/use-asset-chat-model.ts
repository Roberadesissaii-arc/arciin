...
export function useAssetChatModel(
  ...
  /** Image preview: ignore main-chat picks that are not vision-capable or belong to another provider. */
  const preferredForPick = useMemo(() => {
    const p = preferred.trim()
    if (need !== "vision" || !p) return p
    if (profile && !isOllamaChatProfile(profile.provider)) {
      return modelBelongsToProvider(profile.provider, p) &&
        modelNameLooksVision(p)
        ? p
        : ""
    }
  ...
  const activeModel = useMemo(() => {
    ...
    const raw = (explicitModel || modelName).trim()
  ...
  }, [explicitModel, modelName, need, profile])
  ...
  const visionCapable = activeModel
    ? modelSupportsVision(activeModel, showQuery.data ?? null, profile?.provider)
    : false
  ...
  const setupHint = !profile
    ...
    : !activeModel && !modelsLoading
      ...
      : need === "vision" && activeModel && !visionCapable
        ? `“${activeModel}” may not support images. Choose a vision model (e.g. gemini-2.5-flash, ministral-3:3b, gemma3).`
        : null
