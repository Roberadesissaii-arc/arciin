"""
Shared helpers for Arciin Python examples (scripts/examples/*_example.py).

Edit CONFIG below once — individual examples import from here.
"""
...
# Allow `python scripts/examples/foo_example.py` from repo root
...
# WSL: bash scripts/examples/arciin_wsl_hosts.sh → use that IP from Windows
...
def validate_auth_config() -> None:
    ...
    if AUTH_MODE == "api_key":
        ...
        if key in _PLACEHOLDER_API_KEYS:
            raise SystemExit(
                "Replace API_KEY in arciin_example_client.py with your full arc_… secret from Developer → API keys."
            )
        ...
        if not email or email in _PLACEHOLDER_EMAILS:
            raise SystemExit(
                "Replace EMAIL in arciin_example_client.py with your Arciin login (e.g. you@yourdomain.com)."
            )
        if PASSWORD.strip() in ("", "change-me", "your-password", "secret"):
            raise SystemExit("Replace PASSWORD in arciin_example_client.py with your real password.")
